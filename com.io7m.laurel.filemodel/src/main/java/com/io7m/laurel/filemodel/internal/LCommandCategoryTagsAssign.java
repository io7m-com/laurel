...
public final class LCommandCategoryTagsAssign
  extends LCommandAbstract<List<LCategoryAndTags>>
{
  ...
  public LCommandCategoryTagsAssign()
  {
  ...
  public static LCommandFactoryType<List<LCategoryAndTags>> provider()
  {
    return new LCommandFactory<>(
      LCommandCategoryTagsAssign.class.getCanonicalName(),
      LCommandCategoryTagsAssign::fromProperties
    );
  }

  private static LCommandCategoryTagsAssign fromProperties(
    final Properties p)
  {
    final var c = new LCommandCategoryTagsAssign();
    ...
    final List<LCategoryAndTags> categories)
  {
    ...
    for (final var entry : entries) {
      ...
      final var recOpt =
        context.insertInto(TAG_CATEGORIES)
          .set(TAG_CATEGORIES.TAG_CATEGORY_ID, categoryId)
          .set(TAG_CATEGORIES.TAG_TAG_ID, tagId)
          .onConflictDoNothing()
          .returning(
      ...
      if (recOpt.isEmpty()) {
        model.eventWithProgressCurrentMax(
          index,
          max,
          "Category/tag either did not exist or was already assigned.",
          category
        );
        continue;
      }
    ...
    model.eventWithoutProgress("Assigned %d tags.", this.savedData.size());
    ...
    final LDatabaseTransactionType transaction)
  {
    ...
    model.eventWithoutProgress("Assigned %d tags.", Integer.valueOf(max));
    LCommandModelUpdates.updateTagsAndCategories(context, model);
  }
