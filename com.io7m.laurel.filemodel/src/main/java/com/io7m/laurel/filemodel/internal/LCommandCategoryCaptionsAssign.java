...
/**
 * Assign captions to categories.
 */

public final class LCommandCategoryCaptionsAssign
  extends LCommandAbstract<List<LCategoryCaptionsAssignment>>
{
  ...
  public LCommandCategoryCaptionsAssign()
  {
  ...
  public static LCommandFactoryType<List<LCategoryCaptionsAssignment>> provider()
  {
    return new LCommandFactory<>(
      LCommandCategoryCaptionsAssign.class.getCanonicalName(),
      LCommandCategoryCaptionsAssign::fromProperties
    );
  }

  private static LCommandCategoryCaptionsAssign fromProperties(
    final Properties p)
  {
    final var c = new LCommandCategoryCaptionsAssign();
    ...
    final List<LCategoryCaptionsAssignment> categories)
  {
    ...
    for (final var entry : entries) {
      ...
      model.eventWithProgressCurrentMax(
        index,
        max,
        "Assigning caption to category '%s'.",
        category
      );

      final var recOpt =
        context.insertInto(CAPTION_CATEGORIES)
          .set(CAPTION_CATEGORIES.CAPTION_CATEGORY_ID, category.value())
          .set(CAPTION_CATEGORIES.CAPTION_CAPTION_ID, caption.value())
          .onConflictDoNothing()
          .returning(
      ...
      if (recOpt.isEmpty()) {
        model.eventWithProgressCurrentMax(
          index,
          max,
          "Category/caption either did not exist or was already assigned.",
          category
        );
        continue;
      }
  ...
  {
    return "Assign captions to categories";
  }
