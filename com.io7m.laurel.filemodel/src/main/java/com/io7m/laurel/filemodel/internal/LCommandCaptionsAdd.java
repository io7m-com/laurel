...
public final class LCommandCaptionsAdd
  extends LCommandAbstract<List<LCaptionName>>
{
  ...
  public static LCommandFactoryType<List<LCaptionName>> provider()
  {
    return new LCommandFactory<>(
      LCommandCaptionsAdd.class.getCanonicalName(),
      LCommandCaptionsAdd::fromProperties
    );
  }

  private static LCommandCaptionsAdd fromProperties(
    final Properties p)
  {
    final var c = new LCommandCaptionsAdd();
    ...
    for (int index = 0; index < max; ++index) {
      ...
      final var recOpt =
        context.insertInto(CAPTIONS)
          .set(CAPTIONS.CAPTION_TEXT, caption.text())
          .onDuplicateKeyIgnore()
          .returning(CAPTIONS.CAPTION_ID)
          .fetchOptional();
      ...
      this.savedData.add(
        new SavedData(
          rec.get(CAPTIONS.CAPTION_ID).longValue(),
          caption.text()
        )
      );
    }
    ...
    final LDatabaseTransactionType transaction)
  {
    ...
    for (int index = 0; index < max; ++index) {
      ...
      context.insertInto(CAPTIONS)
        .set(CAPTIONS.CAPTION_ID, data.id)
        .set(CAPTIONS.CAPTION_TEXT, data.text)
