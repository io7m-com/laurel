...
public final class LModelOpCaptionsUnassign extends LModelOpAbstract
{
  ...
  public LModelOpCaptionsUnassign(
    ...
    throws LModelOpException
  {
    ...
    for (final var image : this.images) {
      for (final var caption : this.captions) {
        graph.removeEdge(caption, image);

        if (image.captions().contains(caption)) {
          caption.count().set(Math.max(0L, caption.count().get() - 1L));
        }
      }
