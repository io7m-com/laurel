...
public final class LModelOpCaptionCreate extends LModelOpAbstract
{
   ...
   * @param inCaptionID The caption ID
   */

  public LModelOpCaptionCreate(
    ...
    throws LModelOpException
  {
    ...
    if (captions.containsKey(this.captionID)) {
      throw new LModelOpException(
        "Caption %s already exists".formatted(this.captionID)
      );
    }
    ...
    final var caption = new LMCaption(this.captionID);
    caption.setText(this.text);

    this.model.captionTexts().put(this.text, this.captionID);
    this.model.captions().put(this.captionID, caption);
    this.model.imageCaptionGraph().addVertex(caption);
  }
